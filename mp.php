...
$QUERY2="SELECT * FROM BOOK WHERE category='mp'";
...
$image =$row['images'];
        ...
        $check=$row['status'];
if($check=="available")
    echo '<div style =  "width:300px; height:700px; background-color:#eeeeee; text-align:center; float: left; margin: 20px 20px 20px 20px;">'.'<img src = "'.$image.'" ;  height ="300" width= "300"/>'."<br>"."Book Id: ".$row["bid"]."</br>"."Name: ".$row["bname"]."</br>"."Author: ".$row["author"]."</br>"."Category: MP"."</br>"."Status: ".$row["status"]."</br>".'<i>'."If you wish to borrow this book".'<br>'."Fill your user id and book id and click on borrow".'</i>'.'<br>'.'<form  method="post" action="borrow.php">'."Your Id: ".'<input type ="text" name="userid" >'.'<br>'.'<br>'."Book id: ".'<input type ="text" name="bookid" >'.'<br>'.'<br>'.'<input type="submit" value="Borrow">'.'</form>'.'</div>';

else
    echo '<div style =  "width:300px; height:700px; background-color:#eeeeee; text-align:center; float: left; margin: 20px 20px 20px 20px;">'.'<img src = "'.$image.'" ;  height ="300" width= "300"/>'."<br>"."Book Id: ".$row["bid"]."</br>"."Name: ".$row["bname"]."</br>"."Author: ".$row["author"]."</br>"."Category: MP"."</br>"."Status: ".$row["status"]."</br>".'</div>';
}
